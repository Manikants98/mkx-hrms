...
/**
 * Properties for the custom DocumentPicker component
 *
 * @template TFormValues - The Formik form values model
 */
export interface DocumentPickerProps<TFormValues = Record<string, unknown>> {
  ...
  value?: string;
  /** Explicit change callback when a document is selected */
...
/**
 * Custom document file picker component with Formik integration,
 * base64 encoding, and theme-compliant styling.
 *
 * @template TFormValues - The form values model
 * @param props - Configuration properties for the DocumentPicker
 * @returns The rendered DocumentPicker component
 */
export function DocumentPicker<TFormValues = Record<string, unknown>>({
  name,
  label,
  formik,
  value,
  onChange,
  helperText = "PDF or Word (max. 5MB)",
  accept = ".pdf,.doc,.docx",
  ...
  return (
    <Box className="flex flex-col gap-2">
      ...
      <div
        className={`flex items-center gap-4 p-3.5 border rounded-[5px] \${error ? "border-destructive bg-destructive/5" : "bg-secondary/50 dark:bg-black border-border dark:border-zinc-800"}`}
      >
        <div className="!w-14 !h-14 !bg-secondary dark:!bg-zinc-900 !text-foreground !font-semibold !rounded-[5px] !border !border-border dark:!border-zinc-800 shrink-0 flex items-center justify-center">
          <InsertDriveFile className="!w-6 !h-6 text-muted-foreground" />
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!currentDocument ? (
              ...
              <Button
                ...
                className="!text-xs !normal-case !font-medium !border-destructive/30 !bg-card dark:!bg-black !text-destructive hover:!bg-destructive/10 !rounded-[5px] !px-3 !py-1.5"
              >
                Remove
              </Button>
            )}
